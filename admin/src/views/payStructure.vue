  ...
  <div class="article-list">
    ...
    <el-dialog :title="modelTtile" :visible.sync="personModelShow">
      <el-form :model="personObj">
        <el-form-item label="岗位名" label-width="80px">
          <el-input v-model="personObj.name"></el-input>
        </el-form-item>
        <el-form-item label="基础工资" label-width="80px">
          <el-input v-model="personObj.basic"></el-input>
        </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      tableDataAjax() {
        this.$axios
          .get(
            '/payStructure/selectAll?currentPage=' +
            this.pageIndex +
            '&pageSize=' +
            this.pageSize
          )
      ...
      submit() {
        ...
        if (this.isEdit) {
          this.$axios
            .post('/payStructure/update', this.personObj)
            .then(res => {
              if (res.data) {
          ...
          this.$axios
            .post('/payStructure/insert', this.personObj)
            .then(res => {
              if (res.data) {
      ...
      handleDelete(index, row) {
        this.$axios
          .post('/payStructure/delete', [row.id])
          .then(res => {
            if (res.data) {
